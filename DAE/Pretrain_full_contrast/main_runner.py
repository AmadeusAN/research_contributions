...
def parse_option():
    ...
    parser.add_argument("--img_size", default=config.patch_shape[0], type=int, help="image size")
    ...
    parser.add_argument("--embed_dim", default=config.vit_embed_dim, type=int, help="embedding dimention")
    ...
    parser.add_argument("--loss_type", type=str, default="all_img", help="decoder type")
    ...
    parser.add_argument("--warmpup_epoch", default=4, type=int, help="warmup epoch")
    parser.add_argument("--decay_epoch", default=30, type=int, help="warmup epoch")
    parser.add_argument("--save_freq", default=5, type=int, help="saving frequency")
    ...
    parser.add_argument("--choice", default="denoise", type=str, help="choice")
    ...
    parser.add_argument("--temperature", default=0.5, type=float, help="drop path rate")
    parser.add_argument("--mm_con", default=0.03, type=float, help="drop path rate")
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epoch):
        ...
        x_recon = torch.cat(x_recon, dim=0)[:2]
        x_masked = torch.cat(x_masked, dim=0)[:2]
        # if dist.get_rank() == 0:
        x_orig = x_orig[:, :, 48, :, :]
        x_recon = x_recon[:, :, 48, :, :]
        ...
        if val_loss_avg <= val_loss_best:
            save_checkpoint(args, epoch, model, 0.0, optimizer, lr_scheduler, logger, best_model=True)

        # if dist.get_rank() == 0 and (epoch == (args.epoch - 1)):
        if epoch == (args.epoch - 1):
            save_checkpoint(args, epoch, model, 0.0, optimizer, lr_scheduler, logger, best_model=False)
...
def train_one_epoch(args, model, data_loader, optimizer, epoch, lr_scheduler):
    ...
    return loss_meter.avg, loss_l1_meter.avg
...
def validate(data_loader, model):
    ...
    for idx, (img, mask) in enumerate(data_loader):
        ...
        if args.choice == "mae":
            ...
            loss1, img_recon, mask_out = model(img_noisy, mask, img)
    ...
    return loss_meter.avg, loss_l1_meter.avg, img_list, img_recon_list, mask_out_list
